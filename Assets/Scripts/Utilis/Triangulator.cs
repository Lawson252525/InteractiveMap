...
namespace InteractiveMap.Utils {
	...
	public static class Triangulator {
		...
		public struct Triangle {
            ...
            /// <summary>
            /// Цент масс треугольника
            /// </summary>
            public Vector2 center {
                get {
                    var x = (this.a.x + this.b.x + this.c.x) / 3f;
                    var y = (this.a.y + this.b.y + this.c.y) / 3f;
        ...
        /// <summary>
        /// Метод возвращает случайную точку в фигуре из точек
        /// </summary>
        /// <param name="points">Массив точек</param>
        /// <returns>Случайная точка</returns>
        public static Vector2 GetRandomPointWithin(Vector2[] points) {
            var triangles = Triangulate(points.ToList());
            var triangle = triangles[UnityEngine.Random.Range(0, triangles.Count - 1)];
            ...
            var randomPoint = GetRandomPointBetween(onePoint, twoPoint);

            return GetRandomPointBetween(center, randomPoint);
        }

        /// <summary>
        /// Метод возвращает случайную точку на прямой
        /// </summary>
        /// <param name="start"></param>
        /// <param name="end"></param>
        /// <returns></returns>
        public static Vector2 GetRandomPointBetween(Vector2 start, Vector2 end) {
            return (start + UnityEngine.Random.Range(0, 1f) * (end - start));
        }
		/**
		...
		public static List<Triangle> Triangulate(List<Vector2> pathSource, int subsequentCalls = 0) {
			...
			if(!segmentValid) {
				...
				if(subsequentCalls > path.Count) {
					...
					throw new Exception("Cannot triangulate path. Rotated too many times subsequently with no improvements.");
					//return null;
				}
